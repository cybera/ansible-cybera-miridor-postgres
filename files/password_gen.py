...
parser = argparse.ArgumentParser(description='Generate SHA-SCRAM-256 encrypted password')
parser.add_argument('-u', '--username', type=str, help='The username of the user')
parser.add_argument('-p', '--password', type=str, help='The plaintext password you wish to encrypt')
...
def main(username, password):
    ...
    new_pw = EncryptPassword(
        user=username,
        password=password,
        algorithm="scram-sha-256",
    )

    pw = str(new_pw.encrypt())
    print(f"Username: {username}")
    print(f"Password: {pw[2:-1]}")
